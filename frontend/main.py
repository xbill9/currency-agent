...
async def chat_stream(request: SimpleChatRequest):
    ...
    if not target_server_url:
        return {"error": "AGENT_SERVER_URL environment variable not set, and no server_url provided in request"}
    ...
    try:
        agents = await list_agents(target_server_url)
        env_agent_name = os.getenv("AGENT_NAME")
        if env_agent_name and env_agent_name in agents:
            current_agent_name = env_agent_name
        else:
            current_agent_name = agents[0]
        logger.info(f"Using agent: {current_agent_name} on server: {target_server_url}")
    except Exception as e:
        logger.error(f"Failed to list agents on {target_server_url}: {e}")
        current_agent_name = "agent"  # fallback
    ...
    async def event_generator():
        ...
        async for event in events:
            ...
            # Handle progress vs result
            # For this test tool, we'll stream EVERYTHING to final_text
            # and use authors as progress indicators.
            yield (
                json.dumps(
                    {
                        "type": "progress",
